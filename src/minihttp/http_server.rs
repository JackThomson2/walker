...
use crate::minihttp::request::{self, Request};
use crate::minihttp::response::{self, Response};
#[cfg(unix)]
use bytes::Buf;
...
pub trait HttpServiceFactory: Send + Sized + 'static {
  type Service: HttpService + Send + Clone + Copy;
...
fn internal_error_rsp(e: io::Error, buf: &mut BytesMut) -> Response {
  ...
  err_rsp.write_raw_bytes(e.to_string().as_bytes());
  err_rsp
}
