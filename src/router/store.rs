...
use crate::{types::{CallBackFunction, CallbackInner}, Methods};
...
#[cold]
pub fn add_new_route(route: &str, method: Methods, function: CallbackInner) -> Result<()> {
    let lock = GLOBAL_DATA.get_rw_from_method(method);
    let mut writing = lock
        .write();

    if let Ok(found) = writing.at_mut(route) {
        found.value.push(function);
        Ok(())
    } else {
        writing
            .insert(route, vec![function])
